...
import com.extenre.patcher.patch.Option as PatchOption

@Command(
    name = "list-patches",
    description = ["List patches from supplied EXRE files."],
)
internal object ListPatchesCommand : Runnable {
    ...
    @Option(
        names = ["-i", "--index"],
        description = ["List the index of each patch in relation to the supplied EXRE files."],
        showDefaultValue = ALWAYS,
    )
    ...
    override fun run() {
        ...
        fun PatchOption<*>.buildString() =
            buildString {
                appendLine("Title: ${name}")                     // title → name
                description?.let { appendLine("Description: $it") }
                appendLine("Required: $required")
                default?.let {
                    appendLine("Key: ${name}")                   // key → name
                    append("Default: $it")
                } ?: append("Key: ${name}")                      // key → name
